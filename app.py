...
with st.container(border=True):
    ...
    if not df.empty and portfolio_stock_select in df['Stock'].values:
        ...
        with exec_col2:
            ...
            st.markdown(f"**Live Market Entry Price**<br><span style='font-size:20px; font-weight:bold; color:#2ea043;'>₹{current_ticker_live_price:,.2f}</span>", unsafe_allow_html=True)
            
        with exec_col3:
            # Sizing calculation based on selection context
            allowed_loss = user_capital * (risk_pct / 100.0)
            risk_per_share = max((current_ticker_live_price - float(active_stock_data['Risk Exit (SL)'])), 0.01)
        ...
        with exec_col4:
            ...
            if st.button("🚀 Add Asset to Portfolio", use_container_width=True, type="primary"):
                new_trade = {
                    ...
                    "Entry Price": current_ticker_live_price, # Captures current price instantly without manual typing errors
                    "Quantity": int(sim_qty),
                    "Total Outlay": float(current_ticker_live_price * sim_qty),
